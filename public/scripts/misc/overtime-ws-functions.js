...
const addOvertimeEntry = ()=>{
    ...
    $('table#overtime-request-table').find('tr#add-btn-row').before(row);
}
...
const submitOvertimeRequest = () => {
    let overtimeMessageText = $('input#overtime-request-text').val();
    let requestObject = {
        requestMessage: overtimeMessageText,
        overtimeEntries : []
    };

    $('table#overtime-request-table').find('tr#overtime-entry-row').each(function(){
    ...
    let params = 'req&requestObject=' + JSON.stringify(requestObject);

    $.ajax({
        method: 'post',
        url: '/uclm_scholarship/records/submitOvertimeRequest',
        data: params,
        success: res => {
            console.log(res);
            clearRequestForm();
...
const clearRequestForm = ()=>{
    $('input#overtime-request-text').val('');
    $('table#overtime-request-table').find('tr#overtime-entry-row').remove();
}